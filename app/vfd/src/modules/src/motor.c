...
static void motor_param_get(void)
{
    uint8_t value = 0;
    /*启动频率*/
    param_get(PARAM_TYPE2, PARAM2_START_FREQ, &value);
    g_motor_param.start_freq = (float)value;
    /*开高频频率*/
    param_get(PARAM_TYPE2, PARAM2_HIGH_FREQ, &value);
    g_motor_param.open_freq = (float)value;
    /*开高频延时，单位：0.1秒*/
    param_get(PARAM_TYPE1, PARAM1_HIGH_FREQ_DELAY, &value);
    ...
    /*变频关高频*/
    param_get(PARAM_TYPE1, PARAM1_VARI_FREQ_CLOSE, &value);
    g_motor_param.vari_freq_close = value;
    /*转矩提升*/
    param_get(PARAM_TYPE2, PARAM2_TORQUE_BOOST, &value);
    g_motor_param.radio = value;
    /*自动省电*/
    param_get(PARAM_TYPE2, PARAM2_AUTO_ECONOMY, &value);
    g_motor_param.auto_economy = value;
    /*启动方向:0:之前的方向，1:向左 ， 2:向右 */
    param_get(PARAM_TYPE2, PARAM2_START_DIRECTION, &value);
    g_motor_param.start_dir = value;

    /*加速时间，单位0.1秒*/
    param_get(PARAM_TYPE2, PARAM2_ACCE_TIME, &value);
    if(value < 4)
        value = 4;
    g_motor_param.acceleration_time_us = value* 100 * 1000;
    
    /*减速时间，单位0.1秒，经实际测试，50HZ换向时，减速时间最小0.4秒，80Hz换向时，减速时间最小0.7秒*/
    param_get(PARAM_TYPE2, PARAM2_DECE_TIME, &value);
    if(value < 4)
        value = 4;    
...
void motor_high_freq_ctl(int period)
{
    ...
    uint8_t polarity = 0; 
    param_get(PARAM_TYPE1, PARAM1_HIGH_FREQ_POLARITY, &polarity); //获取高频极性
