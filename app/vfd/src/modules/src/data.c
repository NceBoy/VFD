...
static void comm_data_handler(void)
{
    ...
    while(g_uart_buffer_length >= 19)/*消息体为空时，长度为15字节*/
    {
...
static void comm_uart_process(char* buf, int len)
{
    ...
    //    return ;
    ext_send_buf_to_data(0, (unsigned char*)buf, len, MSG_ID_UART_RECV_DATA);
}


int data_process(packet* in , packet* out)
{
    if(in->dtype != TYPE_VFD)
        return -1;
    uint32_t cmd = (in->action << 16) | in->mtype ;
