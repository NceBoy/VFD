...
void motor_start_ctl(void)
{
    ...
    if(g_vfd_ctrl.flag[IO_ID_SP0] != 0) /*手控盒控速*/
    {
        if(g_vfd_ctrl.sp_manual > 10 || g_vfd_ctrl.sp_manual < 8)
            return ;
        param_get(PARAM_TYPE0, g_vfd_ctrl.sp_manual, &speed);
    }
    else 
    {
        if(g_vfd_ctrl.sp > 7)
    ...
    switch(limit)
    {
        case 0x00 : {
            ...
            param_get(PARAM_TYPE2, PARAM2_START_DIRECTION, &dir);
...
static void io_scan_active_polarity(void)
{
    ...
    param_get(PARAM_TYPE1, PARAM1_BUTTON_TYPE, &value); /*点动还是4键控制*/
    g_vfd_ctrl.ctl = (ctl_mode_t)value;

    param_get(PARAM_TYPE1, PARAM1_EXCEED_POLARITY, &value); /*超程信号极性,0:常开 1:常闭*/
    g_vfd_io_tab[IO_ID_LIMIT_EXCEED].active_polarity =  (value == 0 ? ACTIVE_LOW : ACTIVE_HIGH);    

    param_get(PARAM_TYPE1, PARAM1_LEFT_RIGHT_POLARITY, &value); /*左右换向信号极性,0:常开 1:常闭*/
    ...
    param_get(PARAM_TYPE1, PARAM1_STOP_POLARITY, &value); /*加工结束信号极性,0:常开 1:常闭 2:自适应极性*/
...
void inout_mode_sync_from_ext(unsigned char mode)
{
    ...
    else /*退出debug模式*/
    {
        ...
        if(g_vfd_ctrl.flag[IO_ID_SP0] != 0) /*退出debug模式，恢复IO速度*/
        {
            ...
            uint8_t speed = 0;
            param_get(PARAM_TYPE0, g_vfd_ctrl.sp, &speed);
            ext_send_report_status(0,STATUS_SPEED_CHANGE,speed,g_vfd_ctrl.sp);
        }
...
void inout_pump_ctl(int period)
{
    ...
    if(g_pump_ctl.ctrl_delay == 0)
    {
        ...
        ext_send_report_status(0,STATUS_PUMP_CHANGE,g_pump_ctl.ctrl_value,0);
...
static void io_scan_speed(void)
{
    ...
    if((g_vfd_ctrl.flag[IO_ID_SP0] != 0) && (sp_change == 1)) /*用手控盒控制过速度*/
    {   /*通知手控盒取消相关显示*/
        g_vfd_ctrl.flag[IO_ID_SP0] = 0;
    }
    if(motor_is_working())
...
static void io_ctrl_end(void)
{
    ...
    param_get(PARAM_TYPE1, PARAM1_STOP_POSITION, &stop);        
    switch(stop)
    {
        case 1 :{
...
void inout_get_current_sp(unsigned char* sp , unsigned char* value)
{
    if(g_vfd_ctrl.flag[IO_ID_SP0])
    {
        *sp = g_vfd_ctrl.sp_manual;
        param_get(PARAM_TYPE0, g_vfd_ctrl.sp_manual, value);
    }
    else
    {
        *sp = g_vfd_ctrl.sp;
        param_get(PARAM_TYPE0, g_vfd_ctrl.sp, value);
    }
}
...
void scan_voltage(void)
{
    ...
    param_get(PARAM_TYPE2, PARAM2_POWER_OFF_TIME, &power_off_time); /*允许掉电的最长时间，单位0.1秒，最大50*/
