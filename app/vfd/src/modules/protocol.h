...
typedef struct {
    ...
    uint32_t tid;                    // 消息ID (小端)
    uint16_t source_id;              // 源设备ID (小端)
    uint16_t target_id;              // 目标设备ID (小端)
    uint16_t mtype;                  // 消息类型 (小端)
    uint16_t body_length;            // 消息体长度 (小端)
...
void create_packet(packet* packet, ActionCode action, DevType dtype, uint32_t tid,
    uint16_t source_id, uint16_t target_id, uint16_t mtype, uint8_t* body, uint16_t body_length);
